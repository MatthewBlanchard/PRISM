    ...
    for y = 1, game.level.height do
      if fov[x] and fov[x][y] then
        if light[x] and light[x][y] and value(light[x][y]) > .05 then
          ...
          self:write(fov[x][y] == 0 and 261 or 256, x, y, finalColor)
        else
          local amCol = 
          self:write(fov[x][y] == 0 and 261 or 256, x, y, ambientColor)
        end
      elseif explored[x] and explored[x][y] then
        self:write(explored[x][y] == 0 and 261 or 256, x, y, ambientColor)
      end
    end
  end
